...
//MWFunctions
py::class_<FunctionTreeVector<3>> (m, "FunctionTreeVector3D")
    .def(py::init<>())
...
py::class_<BoundingBox<3>> (m, "BoundingBox3D")
...
py::class_<MWTree<3>> mwtree(m, "MWTree3D");
    mwtree
...
py::class_<RepresentableFunction<3>, PyRepresentableFunction<3>> repfunc(m, "RepresentableFunction3D");
    repfunc
    .def(py::init<>())
...
//Gaussians
py::class_<Gaussian<3>> gaussian(m, "Gaussian3D", repfunc);

py::class_<GaussFunc<3>>(m, "GaussFunc3D", gaussian)
